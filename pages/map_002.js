// map_002.js　地図②「桃見山」
// キーワード例: 桃見山、三月、帰れる
// Leaflet共通関数(ensureLeaflet)はmap_001.jsで定義済み

(function() {

  // 桃見山 = 花見山に対応。実座標：福島市花見山公園付近
  const CENTER = [37.7085, 140.5198];
  const ZOOM   = 14;

  // 小蘭が毎年3月に訪れた（遠くから見た）場所と、その記録
  const POINTS = [
    {
  ...
  PAGE_CONTENT['map_002'] = () => {
    setTimeout(() => {
      // ensureLeafletはmap_001.jsで定義。未定義の場合（単独読み込み時）はインライン定義
      const init = () => {
      ...
      if (window._leafletReady && window.L) { init(); }
        ...
        // fallback: map_001未読み込みの場合の最小ensureLeaflet
        const ljs = document.getElementById('_leaflet_js');
        if (ljs) { ljs.addEventListener('load', init); }
        else {
  ...
  <style>
    #leaflet-map-002 {
      ...
      filter:grayscale(30%) brightness(0.70) sepia(25%);
    }
